...
<div class="links">
	<a href="https://github.com/atuttle/Taffy/archive/v2.0.1.zip">Taffy 2.0.1<strong>.zip</strong></a>
	&mdash;
	<a href="https://github.com/atuttle/Taffy/archive/v2.0.1.tar.gz">Taffy 2.0.1<strong>.tar.gz</strong></a>
</div>
...
<br/>[Comparison of REST frameworks for CFML](https://github.com/atuttle/CF-REST-Comparison#coldfusion-rest-comparison)
...
If you **were not** previously using the `applicationStartEvent` or `requestStartEvent` methods, there is no change.

2.0 dropped support for the `applicationStartEvent` and `requestStartEvent` methods in your Application.cfc. Instead, use `onApplicationStart` and `onRequestStart` respectively. You will need to manually call `super.onApplicationStart()` and `super.onRequestStart(arguments.targetPath)` from within your implementations.

* Creating singleton CFC instances (e.g. ColdSpring) that you don't want reinstantiated on every request should be done in `onApplicationStart`, _then_ assigned to variables.framework as needed.
* All changes to `variables.framework` should be made _before_ you call `super.onApplicationStart()`, otherwise they will be ignored.
* Note that `onRequestStart` takes 1 argument (targetPath) and **it is required** that you pass it to `super.onRequestStart`
