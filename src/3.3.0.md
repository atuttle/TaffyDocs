...
#### noData() sends invalid response \([#365](https://github.com/atuttle/Taffy/pull/365)\)

noData() defaults the Content-Type header to `application/json`, but this is invalid because the body is not json. The correct way to do this is to return status 204 with Content-Type `text/plain`. A new method named `noContent()` has been added that does this. Because it would break backwards compatibility if `noData()` behaved this way, a new configuration setting has been added as `variables.framework.noDataSends204NoContent` which defaults to `false`. If you set it to `true` and return `noData()` from a resource, it will behave like `noContent()`.

#### Etag implementation doesn't follow specification \([#395](https://github.com/atuttle/Taffy/pull/395)\)

Etags are supposed to be quoted. While this change to your Etag values will likely invalidate any existing caches, no functionality is lost so we're not considering this a breaking change.
...
If you would otherwise `return rep(someData);` with no additional headers or status code, you may now simply `return someData;`. Just don't forget to add the `rep()` when you need it.

#### Add a realtime search/filter of dashboard resources \([#396](https://github.com/atuttle/Taffy/pull/396)\)

A new input has been added to the dashboard just above the resources list. Type in it to filter the resources. It matches against the URI.
